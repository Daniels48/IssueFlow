...
async def create(project_id: UUID, data: schema.IssueCreate, current_user: CurrentUser, service: issue_service):
...
async def get_all(project_id: UUID, current_user: CurrentUser, service: issue_service, filters: issue_filters):
    return await service.list(project_id=project_id, user=current_user, filters=filters)
...
@router.get("/{issue_id}/edit",response_model=schema.IssueResponseEdit)
...
async def update(issue_id: UUID, data: schema.IssueUpdate, service: issue_service, user: CurrentUser):
